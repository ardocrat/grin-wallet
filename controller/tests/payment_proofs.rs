...
fn payment_proofs_test_impl(test_dir: &'static str) -> Result<(), libwallet::Error> {
	...
	let pp = api1.retrieve_payment_proof(mask1, true, None, Some(slate.id));
	assert!(pp.is_err());
	...
	let mut pp = api1.retrieve_payment_proof(mask1, true, None, Some(slate.id))?;

	println!("Payment proof: {:?}", pp);

	// verify, should be good
	let res = api1.verify_payment_proof(mask1, &pp)?;
